trigger cpeClaim2ContactSunc on CPE_Claim__c (after insert, after update) {
    ...
    List<Contract> lcontract = [Select Id, AccountId, StartDate, EndDate, recordTypeId from Contract where recordTypeId = :CPEContractTypeId and AccountId in :mapAcctIds.keySet() Order By ActivatedDate];
    ...
    for(Contact objContact : [select Id, Name, AccountId from contact where AccountId in: mapAcctIds.keySet()])
    {
        ...
        if(prevContract != null) {
            ...
            system.debug('* * * prevContract: ' + prevContract.Id);
        
            credits=0;
            for(CPE_Claim__c claim : lclaim) {
            
                system.debug('* * * prevContract comp: ' + claim.Candidate_Requirement__r.Candidate_Commitment__c + ':' + claim.ApprovedCredits__c);
            ...
            system.debug('* * * total credits: ' + credits);
